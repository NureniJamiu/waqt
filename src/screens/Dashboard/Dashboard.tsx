...
import { MapPin, Settings as SettingsIcon, History, PlayCircle, ShieldCheck, Clock, Bell, BellOff } from "lucide-react";
          ...
          {prayers.map((prayer) => (
            <div
              ...
              className={`glass-panel p-4 rounded-2xl flex flex-col justify-between transition-all ${
                prayer.isNext
                  ? "border-emerald-500/50 bg-emerald-950/20 ring-2 ring-emerald-500/20 shadow-lg shadow-emerald-500/5"
                  : prayer.isPassed
      ...
      {onTriggerTestOverlay && (
        <div className="pt-4 flex justify-between items-center">
          <div className="text-xs text-slate-500">
            Forced Pause: <strong className="text-slate-400">{Math.round(settings.forcedPauseSeconds / 60)} mins</strong> • Snooze: <strong className="text-slate-400">{settings.snoozeEnabled ? "Enabled" : "Disabled"}</strong>
          </div>
          <button
